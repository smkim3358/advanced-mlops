...
class Preprocessor:
    ...
    def _transform_with_robust_scaler(
        ...
        for feature in features:
            scaler = RobustScaler() # 1. 각 피처마다 RobustScaler()를 생성하여 적합시키고 
            robust_scalers[feature] = scaler.fit(x_train[[feature]]) # 2. 딕셔너리 robust_scalers에 저장 (key=피처이름, value=scaler)
            x_train[feature] = scaler.transform(x_train[[feature]]) # 3. 각 학습 데이터와 검증 데이터의 피처에 적용하여 대체
